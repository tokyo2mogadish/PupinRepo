...
def Validate(Zahtev):
    try:
       ...
       #Provera da li lice odgovorno za potpisivanje
        ...
        #00000-3
        if not( suma(bs,1,25,5)+suma(bs,1,25,6)+suma(bs,1,25,7)+suma(bs,401,428,5)+suma(bs,401,428,6)+suma(bs,401,428,7)+suma(bu,1001,1048,5)+suma(bu,1001,1048,6) > 0 ):
        ...
        if not(proveriNapomene(bsNapomene,1,25,4) and proveriNapomene(bsNapomene,401,428,4) and proveriNapomene(buNapomene,1001,1048,4)): 
            
            naziv_obrasca='Bilans stanja'
            poruka  ='Na AOP-u (0001 do 0025) bilansa stanja + (0401 do 0428) bilansa stanja + (1001 do 1048) bilansa uspeha u koloni 4 (Napomena broj) ne sme biti iskazan podatak kod obveznika koji nemaju obavezu dostavljanja napomena uz finansijski izveštaj. '
            aop_pozicije=[]
            poruka_obrasca = {'naziv_obrasca': naziv_obrasca, 'poruka': poruka, 'aop_pozicije': aop_pozicije}
            form_errors.append(poruka_obrasca)
            naziv_obrasca='Bilans uspeha'
